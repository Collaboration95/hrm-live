...
concrete issues. Every row below was resolved in the v0.9.0 release; the table
is retained as decision evidence rather than live status.
...
- Raise the coverage floor above its current 60% in deliberate increments,
  beginning with presentation logic, failure states, config migrations, and BLE
  reconnection.
...
- Add screenshot-based visual regression tests for dashboard and settings in
  light/dark mode at normal and larger accessibility text sizes. Approved
  screenshots captured from each release candidate are the baseline; re-capture
  and re-approve whenever the UI changes.
